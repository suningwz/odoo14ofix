...
class HrEmployeeTransferWiz(models.TransientModel):
    ...
    registro_patronal_id = fields.Many2one(relation='hr.ext.mx.regpat',
                                           related='branch_id.registro_patronal',
                                           string='Employer Registration', readonly=True)
    company_dest_id = fields.Many2one(
        'res.company', string='Destination Warehouse')
    
    branch_dest_id = fields.Many2one('company.branch',string='Destination Branch',required=True)
    registro_patronal_dest_id = fields.Many2one(relation='hr.ext.mx.regpat',
                                                related='branch_dest_id.registro_patronal',
                                                string='Employer Registration', readonly=True)
    ...
    def buscar_nominas(self):
        ...
        nomina_bimestral_actual = self.env['hr.payslip'].search([
            ...
            ('registro_patronal_codigo', '=',
             self.branch_id.registro_patronal.name),
    ...
    def cambiar(self):
        ...
        self.env['hr.employee.transfer'].create({
            ...
            'branch_dest_id':self.branch_dest_id.id,
            'wage': self.wage,
        })

        # Actualiza datos en el empleado
        if self.branch_id.company_id.id != self.branch_dest_id.company_id.id:
            address = self.branch_dest_id.company_id.partner_id.address_get(['default'])
            ...
            self.employee_id.company_id = self.branch_dest_id.company_id
        self.employee_id.registro_patronal = self.registro_patronal_dest_id
        self.employee_id.branch_id = self.branch_dest_id.id
        ...
        # Actualiza datos en el contrato vigente
        if self.branch_id.company_id.id != self.branch_dest_id.company_id.id:
            self.contract_id.company_id = self.branch_dest_id.company_id
