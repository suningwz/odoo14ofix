...
class HrGenLayPolizaWiz(models.TransientModel):
    ...
    def gen_laypoliza(self):
        ...
        # Hojas de trabajo
        if not data:
            sheet = wbk.add_sheet("CargaNomina", cell_overwrite_ok=True)
        for dhoja in data:
    ...
    def get_poliza_data(self, payslips):
        ...
        for payslip in payslips:
            ...
            company_id = payslip.employee_id.branch_id and payslip.employee_id.branch_id.id or ''
            company_name = payslip.employee_id.branch_id and payslip.employee_id.branch_id.name or ''
            ...
            self.add_to_dict(company_id, company_name, data_dict, lineas)
