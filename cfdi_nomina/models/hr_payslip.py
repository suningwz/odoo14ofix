...
class HrPayslip(models.Model):
    ...
    def action_payslip_done(self):
        ...
        for payslip in self:
            if payslip.move_id:
                payslip.move_id.branch_id = payslip.employee_id and payslip.employee_id.branch_id and payslip.employee_id.branch_id.id or False 
            total = 0
    ...
    def compute_sheet(self):
        ...
        for payslip in self:
            ...
            other_payslip_ids = self.env['hr.payslip'].search([('state','!=','cancel'),('id','!=',payslip.id),('employee_id','=',payslip.employee_id.id)])
            #if integ_factor > 0:
            if other_payslip_ids:
            ...
            nomina_bimestral = self.env['hr.payslip'].search([
                ...
                ('registro_patronal_codigo', '=',
                 payslip.employee_id.branch_id.registro_patronal.name),
            ])
            if nomina_bimestral:
    ...
    def calc_cuotas_obrero_patronal(self, uma=False):
        for payslip in self:
            if not payslip.employee_id.branch_id.registro_patronal:
                raise UserError('No hay Registro patron en la branch %s del empleado %s' % (
                    payslip.employee_id.branch_id.name, payslip.employee_id.nombre_compelto))

            rp = payslip.employee_id.branch_id.registro_patronal
            if not uma:
                uma = rp.UMA
    ...
    def calculate_sdi(self, rule_localdict):
        ...
        if tipo_sueldo in ['VARIABLE', 'MIXTO', '']:
            ...
            nomina_bimestral = self.env['hr.payslip'].search([
                ...
                '|',
                ('registro_patronal_codigo', '=',
                 employee.branch_id.registro_patronal.name),
